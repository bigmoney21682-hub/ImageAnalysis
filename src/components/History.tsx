...
function provenance(entry: HistoryEntry): string {
  return [when(entry.at), entry.model || entry.provider].filter(Boolean).join(' · ')
}
...
export function History({ onClose, providerId, apiKey, model }: Props) {
  ...
  if (open) {
    // The full-resolution image was never stored, so follow-ups here run against
    // the thumbnail. Said out loud, because it genuinely changes what can be
    // answered about fine detail.
      ...
      <div className="history">
        ...
        >
          <Chat
            ...
            note={
              image
                ? 'Answers here run against the stored thumbnail, not the original. Fine detail may not survive it.'
                : undefined
            }
          />
        </Results>
    ...
    <div className="history">
      ...
      {entries.length === 0 ? (
        <p className="history__empty">
          Nothing saved yet. Every image you analyze is kept here automatically — tap one to see
          the full report again, or to carry on asking about it.
        </p>
      ) : (
        <>
              ...
              <li key={entry.id}>
                <button className="hitem" onClick={() => setOpenId(entry.id)}>
                  ...
                  <span className="hitem__text">
                    <span className="hitem__type">{entry.modality || 'Image'}</span>
                ...
                <button
                  ...
                  aria-label={`Delete report of ${entry.modality || 'this image'}`}
                >
                  ✕
                </button>
              </li>
      ...
      <p className="history__note">
        Saved in this browser only, on this device. Images are kept as small thumbnails; the
