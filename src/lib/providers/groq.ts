 ...
 * on a report someone is going to read clinically.
 *
...
function friendlyError(status: number, body: string, model = DEFAULT_MODEL): string {
  ...
  if (status === 413) return 'The image is too large for Groq. Try a tighter crop.' + suffix
...
async function runAnalyze(
  ...
  // Nothing here is guaranteed by the API, so every optional array is defaulted.
  return {
    ...parsed,
  ...
  const res = await post(
    ...
    signal,
    {
      ...
      messages: [
        ...
        {
          role: 'assistant',
          content: `My structured report on this image:\n${JSON.stringify(input.analysis)}`,
        },
  ...
  async analyze(input, { apiKey, model, signal, onModel, onCredential }): Promise<Analysis> {
    return withFallbackChain({
      ...
      onFallback: ({ from, to, reason }) =>
        console.info(`[groq] ${from} refused this image, retrying on ${to}. ${reason}`),
