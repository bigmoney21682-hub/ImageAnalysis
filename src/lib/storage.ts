...
const KEY_PROVIDER = 'img.provider'
const KEY_HISTORY = 'img.history'
const KEY_CONSENT = 'img.consent'

/** Keys are per-provider, so adding a second provider later costs nothing. */
const apiKeyFor = (providerId: string) => `img.apiKey.${providerId}`
const modelFor = (providerId: string) => `img.model.${providerId}`
...
/**
 * Uploading a patient image to a third-party API is a decision with real
 * consequences, so it gets made once, deliberately, rather than buried in a
 * footer nobody reads.
 */
export const consentStore = {
  get: () => localStorage.getItem(KEY_CONSENT) === '1',
  set: (v: boolean) => {
 ...
 * dev server is reached over plain http on a LAN address. These ids are just
 * local list keys — uniqueness is all that matters, not cryptographic quality.
 */
...
export const historyStore = {
  ...
  add(input: HistoryInput): HistoryEntry {
    const entry: HistoryEntry = {
      id: historyId(),
      at: Date.now(),
      modality: input.analysis.study?.modality ?? 'Image',
