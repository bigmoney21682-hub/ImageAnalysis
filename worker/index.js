/**
 * Gemini proxy for the Image Analysis app.
 *
 ...
 * reaching a browser.
 *
 * The cost of that convenience: every image analysed through this Worker passes
 ...
 * it must stay that way — a single console.log of the body would put patient
 * images into your log tail.
 */
...
export default {
  async fetch(request, env) {
    ...
    if (!env.GEMINI_KEY)
      return fail(500, 'The proxy has no GEMINI_KEY secret set.', request, env)
    if (!env.APP_TOKEN)
      return fail(500, 'The proxy has no APP_TOKEN secret set.', request, env)
