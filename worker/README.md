...
The app's default is to call Google directly from the browser. No image touches
any server you run, which for medical images is a deliberate property and not an
accident of the architecture.

This Worker gives that up. Every image analysed through it passes through your
...
if you also want the dev server to reach it.
...
that URL, enter the passphrase, Save. Hit **Test key & list models** — it should
list models without any API key in the key field.
...
Gemini key behind it is the thing with a real quota. Rotate the passphrase by
running `wrangler secret put APP_TOKEN` again; every client then needs the new
one, which is the intended blast radius.
